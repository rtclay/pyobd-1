...
class OBD_App(wx.App):
    ...
    class sensorProducer(threading.Thread):
        ...
        def init_communication(self):
            ...
            for i in range(1, len(self.supp)):
                if self.supp[i - 1] == "1":  # put X in column if PID is supported
    ...
    def OnHelpAbout(self, event):  # todo about box
        Text = """  PyOBD is an automotive OBD2 diagnostic application using ELM237 cable.
...
#            tree.AppendItem(leaf, ucodes[c])
    ...
    def scanSerial(self):
        ...
        for i in range(256):
            try:  # scan standard ttyS*
                ...
                available.append(serial_port.portstr)
                serial_port.close()  # explicit close 'cause of delayed GC in java
            except serial.SerialException:
                pass
        for i in range(256):
            try:  # scan USB ttyACM
                serial_port = serial.Serial("/dev/ttyACM" + str(i))
                available.append(serial_port.portstr)
                serial_port.close()  # explicit close 'cause of delayed GC in java
            except serial.SerialException:
                pass
        for i in range(256):
            try:
                serial_port = serial.Serial("/dev/ttyUSB" + str(i))
                available.append(serial_port.portstr)
                serial_port.close()  # explicit close 'cause of delayed GC in java
            except serial.SerialException:
                pass
        for i in range(256):
            try:
                serial_port = serial.Serial("/dev/ttyd" + str(i))
                available.append(serial_port.portstr)
                serial_port.close()  # explicit close 'cause of delayed GC in java
            except serial.SerialException:
                pass
        ...
        if len(platform.mac_ver()[0]) != 0:  # search only on MAC
            for i in range (65535):
                ...
                try:
                    serial_port = serial.Serial("/dev/tty.usbmodem" + extension)
                    available.append(serial_port.portstr)
                    serial_port.close()
                except serial.SerialException:
                    pass
    ...
    def Configure(self, e=None):
        dialog_id = 0
        diag = wx.Dialog(self.frame, dialog_id, title="Configure")
        sizer = wx.BoxSizer(wx.VERTICAL)

        ports = self.scanSerial()
        rb = wx.RadioBox(diag, dialog_id, "Choose Serial Port",
